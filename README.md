...
- 异常捕获依靠标准库导出的 `error` 变量, 所有错误抛出时都会赋值给这个变量.
- 异常抛出可以使用 `throw` 关键字.
- 包裹在 `catch` 代码块中的代码抛出异常中断后不影响代码块外部代码的执行.

eg:

```
from "std" import { error };
...
export declare struct A { a: I32, b: I32 };
